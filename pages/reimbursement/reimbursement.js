Page({
    ...
    createMessage(reimbursementId) {
      ...
      db.collection('Messages').add({
        ...
        success: () => {
          console.log('消息已成功添加');
          // 消息添加成功后，调用云函数Trigger_approvals
          this.callCloudFunction(reimbursementId);
        },
        fail: (err) => {
    ...
    // 调用云函数Trigger_approvals
    callCloudFunction(reimbursementId) {
      wx.cloud.callFunction({
        name: 'Trigger_approvals', // 云函数名称
        data: {
          reimbursementId: reimbursementId // 传递给云函数的参数
        },
        success: res => {
